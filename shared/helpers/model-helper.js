        ...
        Object.keys(models).forEach((modelName) => {
            ...
            if (model.rawAttributes) {
                Object.keys(model.rawAttributes).forEach((attrName) => {
                    ...
                    if (basicType) {
                        logger.trace(
                            `Adding edge from ${model.name.toLowerCase()} to ${basicType} with name ${attrName}`,
                        );
            ...
            if (model.associations) {
                Object.keys(model.associations).forEach((assocName) => {
                    ...
                    logger.trace(
                        `Adding edge from ${model.name.toLowerCase()} to ${targetName} with name ${assocName} (multiple: ${fromMultiple})`,
                    );
