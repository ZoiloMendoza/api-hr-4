...
const rfcValidator = Joi.string()
    ...
    .messages({
        'string.rfcFormat':
            'Invalid RFC format. Must be 12 characters for business entities or 13 characters for individuals',
        'string.pattern.base': i18n.__('invalid.fieldFormat', 'RFC'),
    });

const licenseNumberValidator = Joi.string()
    .pattern(/^[A-Z0-9]{8,15}$/)
    .messages({
        'string.pattern.base': 'El número de licencia debe tener entre 8 y 15 caracteres alfanuméricos en mayúsculas.',
    });

module.exports = { rfcValidator, licenseNumberValidator };
